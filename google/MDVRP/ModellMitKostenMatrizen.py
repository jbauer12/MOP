...
def costs_for_cargoType1(from_index, to_index, data):
    ...
    return getBenzinCosts(data['distance_matrix'][from_index][to_index], 10.3) + getCostsPerPaket(
        stundensatz_mitarbeiter,
        zeit_pro_paket_in_std) * pakete + getCostsPerAbgabeStelle() + getCostFromWorkerPerKM(
        data['distance_matrix'][from_index][to_index], stundensatz_mitarbeiter)
...
def costs_for_cargoType2(from_index, to_index, data):
    ...
    # Convert from routing variable Index to distance matrix NodeIndex.
    pakete = data['demands'][to_index]
    return getBenzinCosts(data['distance_matrix'][from_index][to_index], 11.1) + getCostsPerPaket(
        stundensatz_mitarbeiter,
        zeit_pro_paket_in_std) * pakete + getCostsPerAbgabeStelle() + getCostFromWorkerPerKM(
        data['distance_matrix'][from_index][to_index], stundensatz_mitarbeiter)
...
def costs_for_cargoType3(from_index, to_index, data):
    ...
    # Convert from routing variable Index to distance matrix NodeIndex.
    pakete = data['demands'][to_index]
    return getBenzinCosts(data['distance_matrix'][from_index][to_index], 14.2) + getCostsPerPaket(
        stundensatz_mitarbeiter,
        zeit_pro_paket_in_std) * pakete + getCostsPerAbgabeStelle() + getCostFromWorkerPerKM(
        data['distance_matrix'][from_index][to_index], stundensatz_mitarbeiter)
...
def main():
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        elif vehicle_id % (len(data["vehicle_capacities"]) // 7) <= 20:
            routing.SetArcCostEvaluatorOfVehicle(transit_callback_matrix_c2, vehicle_id)
            routing.SetFixedCostOfVehicle(data['fixcostsyearAndDay'][1], vehicle_id)
        else:
            routing.SetArcCostEvaluatorOfVehicle(transit_callback_matrix_c3, vehicle_id)
